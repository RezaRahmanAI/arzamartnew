...
const INITIAL_CITY_AREAS_MAP: Record<string, string[]> = {
  Dhaka: [
...
export default function AdminPreOrderPage() {
  ...
  const clearCart = () => {
    setLines([]);
    toast.success("Pre-order items cleared");
  };

  // Open select options modal popup
  ...
  const handleCreatePreOrder = (e: React.FormEvent) => {
    ...
    if (lines.length === 0) {
      toast.error("Cart is empty! Search and add products to pre-order.");
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch">
        {/* LEFT COLUMN: Cart Table, Summary & Form */}
        <div className="space-y-4">
          <form onSubmit={handleCreatePreOrder} className="space-y-4">
            ...
            {/* Customer Info & Pre-Order Details */}
            <div className="rounded-xl border border-border bg-card shadow-card p-4 space-y-3">
              <h2 className="font-bold text-sm text-foreground border-b border-border/60 pb-1.5 flex items-center justify-between">
                <span>Customer & Pre-order Dispatch Info</span>
              </h2>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label htmlFor="po-phone" className="text-xs font-semibold">Phone Number *</Label>
                  <Input
                    id="po-phone"
                    type="tel"
                    value={phone}
                ...
                <div className="space-y-1">
                  <Label htmlFor="po-customer" className="text-xs font-semibold">Customer Name *</Label>
                  <Input
                    id="po-customer"
                    value={customer}
              ...
              <div className="space-y-1">
                <Label htmlFor="po-address" className="text-xs font-semibold">Address *</Label>
                <Textarea
                  id="po-address"
                  value={address}
              ...
              {/* Expected Dispatch Date (Pre-Order Specific) */}
              <div className="space-y-1 p-2.5 bg-amber-500/10 rounded-lg border border-amber-500/20">
                <Label htmlFor="po-dispatch" className="text-xs font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
                  <Calendar className="size-3.5" /> Expected Dispatch Date
                </Label>
                <Input
                  id="po-dispatch"
                  type="date"
        ...
        {/* RIGHT COLUMN: POS Product Search & Catalog Grid (EXACT SAME DESIGN AS MANUAL ORDER) */}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="absolute top-1.5 left-1.5 z-10">
                          <span className="inline-flex items-center rounded bg-amber-600/95 text-white px-1.5 py-0.5 text-[9px] font-extrabold shadow-xs">
                            Pre-order
                          </span>
                        </div>
                          ...
                          <div className="grid grid-cols-3 gap-0.5">
                            {product.sizes.slice(0, 6).map((sz) => (
                              ...
                              >
                                ...
                                <span className="text-[8.5px] text-amber-700 font-semibold">+</span>
                              </button>
                            ))}
                          </div>
                        </div>
